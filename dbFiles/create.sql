CREATE TABLE IF NOT EXISTS employee_personal_info(
    ...
    phone           TEXT    CHECK (phone GLOB '???-???-????')   NOT NULL,
...
CREATE TABLE IF NOT EXISTS badge_info(
    ...
    FOREIGN KEY (employee_id) REFERENCES employee_personal_info(employee_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS employee_company_info(
    ...
    FOREIGN KEY (manager_emp_id) REFERENCES employee_personal_info(employee_id)
);
